...
_NET_CONFIG_INFO  NetConfigInfo[XY_MAX] =
{
    ...
    {XY_HY,		{116,62,125,35},8000, {"116.62.125.35"},	1},  //汇誉IP端口
#elif(HY_Cover_XY == show_ANC)
    {XY_HY,		{47,108,227,112},8000, {"47.108.227.112"},	1},  //安充IP端口
#endif

#if(YKC_Cover_XY == show_SY)
	{XY_YKC,	{81,70,61,150},50000,{"81.70.61.150"}, 1},  //塑云 IP
#elif(YKC_Cover_XY == show_YKC)
    {XY_YKC,	{121,43,69,62},8767,{"121.43.69.62"}, 1},  //YKC IP
#elif(YKC_Cover_XY == show_DKY)
    {XY_YKC,	{101,42,126,178},50000,{"101.42.126.178"}, 1},  //达克云 IP
#endif
...
{ 
	#define RESET_4G_COUNT	7    //目前连续7次未连接上服务器，则重新启动
...
extern void GotoUserApplication(void);
#define MSG_NUM    5
/*****************************************************************************
* Function     : Task4GMain
* Description  : 4G主任务
...
void Task4GMain(void *pdata)
{
    ...
    rt_pin_write(IO_4G_RES,PIN_LOW);   //复位关闭
    rt_thread_mdelay(250);


    rt_pin_write(IO_4G_RES,PIN_HIGH);   //复位开启
    ...
    while(1)
    {
        ...
        else
        {
            ...
            {
                //memcpy(HttpInfo.ServerAdd,"https://cpile.oss-cn-hangzhou.aliyuncs.com/cpile/2022-12-06/project.bin",strlen("https://cpile.oss-cn-hangzhou.aliyuncs.com/cpile/2022-12-06/project.bin"));
                memcpy(HttpInfo.ServerAdd,"http://hy.shuokeren.com/uploads/AC1-205.bin",strlen("http://hy.shuokeren.com/uploads/AC1-205.bin"));

                if(Module_HTTPDownload(&HttpInfo))
                {
                    download_count = 0;
                    HY_SendUpdata_flag(0x00); //更新升级成功0x00
                ...
                else
                {
                    if(++download_count > 3)
                    {
                        ...
                        APP_SetSIM7600Mode(MODE_DATA);
                        Send_AT_CIPMODE();
                        HY_SendUpdata_flag(0x03); //更新升级超时0x03
                        rt_thread_mdelay(1000);
                    }
                }
            }
