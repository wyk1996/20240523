...
uint8_t FlashDisposeBuf[3000];				//flash处理缓存
...
uint8_t   AP_SendFrameDispose(void)
{
    ...
    for(i = 0; i < AP_SEND_FRAME_LEN; i++)
    {
        ...
        APSendTable[i].curtime = rt_tick_get();
...
static uint8_t  AP_SendChargeInfoA(void)
{
    ...
    if(stChTcb.ucState == CHARGING)
    {
        ...
        {
            APSendB1[GUN_A].dc_charge_voltage = stChTcb.stRn8209.usVolt;			//电压
            APSendB1[GUN_A].dc_charge_current  = stChTcb.stRn8209.usCurrent;	//电流  精确到小数点后二位
        }
        else
        {
        ...
        APSendB1[GUN_A].ChargeJKwh = (stChTcb.stCh.uiaChargeEnergy[0]*10) / (stChRate.Prices[0] + stChRate.fwPrices[0]);
        APSendB1[GUN_A].ChargeFKwh = (stChTcb.stCh.uiaChargeEnergy[1]*10) / (stChRate.Prices[1] + stChRate.fwPrices[1]);
        APSendB1[GUN_A].ChargePKwh = (stChTcb.stCh.uiaChargeEnergy[2]*10) / (stChRate.Prices[2] + stChRate.fwPrices[2]);
        APSendB1[GUN_A].ChargeGKwh = (stChTcb.stCh.uiaChargeEnergy[3]*10) / (stChRate.Prices[3] + stChRate.fwPrices[3]);
        APSendB1[GUN_A].ChargeTime = (time(RT_NULL) - stChTcb.stCh.timestart) / 60;		//累计充电时间	单位：min
...
uint8_t  AP_SendBill(_GUN_NUM gun)
{
    ...
    if(DisSysConfigInfo.energymeter == 1)
    {
        APSendBilB12[gun].ChargeStartKwh = (stChTcb.stRn8209.uiE - stChTcb.stCh.uiChargeEnergy) / 10; //充电前电表读数
        APSendBilB12[gun].ChargeStopKwh =stChTcb.stRn8209.uiE / 10;  ;
    }
    else
    {
...
uint8_t  PreAPOffLineBill(_GUN_NUM gun,uint8_t *pdata)
{
    ...
    if(DisSysConfigInfo.energymeter == 1)
    {
        APSendBilB15[gun].ChargeStartKwh = (stChTcb.stRn8209.uiE > stChTcb.stCh.uiChargeEnergy) ? \
                                           (stChTcb.stRn8209.uiE - stChTcb.stCh.uiChargeEnergy): (stChTcb.stRn8209.uiE + RN8209D_MAX_E - stChTcb.stCh.uiChargeEnergy); //充电前电表读数
        APSendBilB15[gun].ChargeStopKwh = stChTcb.stRn8209.uiE / 10;
    }
    else
    {
    ...
    APSendBilB15[gun].ChargeJKwh = (stChTcb.stCh.uiaChargeEnergy[0]*10) / (stChRate.Prices[0] + stChRate.fwPrices[0]);
    APSendBilB15[gun].ChargeFKwh = (stChTcb.stCh.uiaChargeEnergy[1]*10) / (stChRate.Prices[1] + stChRate.fwPrices[1]);
    APSendBilB15[gun].ChargePKwh = (stChTcb.stCh.uiaChargeEnergy[2]*10) / (stChRate.Prices[2] + stChRate.fwPrices[2]);
    APSendBilB15[gun].ChargeGKwh = (stChTcb.stCh.uiaChargeEnergy[3]*10) / (stChRate.Prices[3] + stChRate.fwPrices[3]);

    APSendBilB15[gun].JMoney = stChTcb.stCh.uiaChargeEnergy[0]/10000;
    APSendBilB15[gun].FMoney = stChTcb.stCh.uiaChargeEnergy[1]/10000;
    APSendBilB15[gun].PMoney = stChTcb.stCh.uiaChargeEnergy[2]/10000;
    APSendBilB15[gun].GMoney = stChTcb.stCh.uiaChargeEnergy[3]/10000;
...
uint8_t  PreAPFSOffLineBill(_GUN_NUM gun,uint8_t *pdata)
{
    ...
    for(i = 0; i <  stChRate.TimeQuantumNum; i++)
    {
        ...
        APSendBilB53[gun].PricInfo[i].ServeFee = stChTcb.stCh.uifsfwChargeEnergy[i]/10000;
        APSendBilB53[gun].PricInfo[i].Price = (stChTcb.stCh.uifsChargeEnergy[i] - stChTcb.stCh.uifsfwChargeEnergy[i])/10000;
        if(CurQuan < 4)
        {
            APSendBilB53[gun].PricInfo[i].Power = stChTcb.stCh.uifsChargeEnergy[i] *10 / stChRate.Prices[CurQuan];
        }
...
uint8_t  AP_SendVinWLAck(void)
{
    ...
    memcpy(&Sendbuf[15],(uint8_t*)&APSendVinB20,sizeof(APSendVinB20));
...
static uint8_t   AP_RecvCardB7(uint8_t *pdata,uint16_t len)
{
    ...
    {
        //通知鉴权失败
        rt_pin_write(70, PIN_HIGH );
        rt_thread_mdelay(100);
        rt_pin_write(70, PIN_LOW);
    }
...
static uint8_t AP_RecvB16OFFLineRco(uint8_t *pdata,uint16_t len)
{
    ...
    if(CmpNBuf(poffline->DrderNum,APOFFLineRec.DrderNum,sizeof(poffline->DrderNum) ) == RT_TRUE)
    {
        //订单号一致
...
{
    uint8_t i = 0,recvstate = 0;  //recvstate主要是判断是否真正的接收到数据
    uint8_t num;
