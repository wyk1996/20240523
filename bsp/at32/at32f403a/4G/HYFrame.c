...
uint8_t   HY_SendDevStateA(void)
{
    ...
    if(SendHYCmd104[GUN_A].work_stat == 2)
    {
        ...
        else
        {
            ...
            SendHYCmd104[GUN_A].ac_a_vol = dlt645_info.out_vol*10;			//电压
            SendHYCmd104[GUN_A].ac_a_cur  =  dlt645_info.out_cur*10;	//电流
        }
...
/***************************************************************
**Function   :HY_SendUpdata_flag
**Description: 远程升级设置标志位
**Input      :flag: [输入/出]
**Output     :
**Return     :
**note(s)    :
...
***************************************************************/
uint8_t  HY_SendUpdata_flag(uint8_t flag)
{
    SendHYCmd1102.UpdataState = flag;
    return RT_TRUE;
}

/***************************************************************
**Function   :HY_ReturnUpdata_flag
**Description: 远程返回标志位
**Input      :None
**Output     :
**Return     :
...
**Create_Time:2022-12-13
***************************************************************/
uint8_t  HY_ReturnUpdata_flag(void)
{
    return SendHYCmd1102.UpdataState;
}
...
uint8_t   HY_RecvFrameDispose(uint8_t * pdata,uint16_t len)
{
    ...
    //数据分帧   /**加一个whil 防止包与包之间相粘，不加这个一定会应答不了**/
    pframedata = pdata;
    while(len > 15)
    {
        //帧头帧尾判断
...
static uint8_t   HY_RecvStartCharge(uint8_t *pdata,uint16_t len)
{
    ...
    ResendBillControl[GUN_A].ResendBillState = RT_FALSE;	  //之前的订单无需发送了
...
//    }
    ...
    HY_SendUpdata_flag(0x05);		//先赋值一个标志位
    return TRUE;
