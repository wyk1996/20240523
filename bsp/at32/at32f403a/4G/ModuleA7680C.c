...
extern const struct fal_partition *app_code;				//代码段
...
*****************************************************************************/
static uint8_t get_ota_crc_Data(uint8_t basedata,uint8_t *pbuf,uint16_t datalong)
{
    uint16_t i;
    uint8_t data = basedata;
    ...
    while(!SIM7600CmdState.HTTP_ReadDone)
    {
        ...
        while (i < CM_TIME_2_SEC / CM_TIME_50_MSEC)
        {
            ...
            if (SIM7600CmdState.HTTP_Read== STATE_OK)
            {
                ...
                //数据存储
                fal_partition_erase(app_code, down_len + 5, pcontrol->DownPackLen);
...
uint8_t ModuleSIM7600_SendData(uint8_t num,uint8_t* pdata,uint16_t len)
{
    ...
    if(NetConfigInfo[NET_YX_SELCT].NetNum > 1)
    {
        if((num > 10) || (pdata == NULL) )
        {
            rt_mutex_release(dynamic_mutex);//释放锁
            return FALSE;
        }
        Uart4GCom.write(buf, strlen((char*)buf) );
        rt_thread_mdelay(CM_TIME_50_MSEC);
