...
_4G_START_TYPE YKCStartType[GUN_MAX] = { _4G_APP_START,_4G_APP_START};					//YKCC 0表示App启动 1表示刷卡启动
...
uint8_t   YKC_SendFrameDispose(void)
{
    ...
    for(i = 0; i < YKC_SEND_FRAME_LEN; i++)
    {
        ...
        YKCSendTable[i].curtime = rt_tick_get();
...
uint8_t   YKCFreamSend(uint8_t cmd,uint8_t *pdata, uint16_t len)
{
    ...
    rt_kprintf("tx len:%d,tx data:",8+len);
    for(i = 0; i < 8+len; i++)
    {
...
uint8_t   YKC_SendHearGunACmd3(void)
{
    ...
    {
        YKCSendCmd3[GUN_A].GunState  = 0x00;   //正常 00
    }
    else
    {
        YKCSendCmd3[GUN_A].GunState  = 0x01;	//故障 01
    }
...
uint8_t   YKC_SendSJDataGunACmd13(void)
{
    ...
    if(stChTcb.ucState == CHARGING)
    {
        ...
        {
            YKCSendCmd13[GUN_A].OutVol = stChTcb.stRn8209.usVolt;			//电压		//输出电压  0.1
            YKCSendCmd13[GUN_A].OutCur = stChTcb.stRn8209.usCurrent/10;		//输出电流	0.1
        }
        else
        {
        ...
        YKCSendCmd13[GUN_A].ChargeTime = (time(RT_NULL) - stChTcb.stCh.timestart) /60;	//充电时间  min
...
uint8_t   PreYKCBill(_GUN_NUM gun,uint8_t *pdata)
{
    ...
    if(DisSysConfigInfo.energymeter == 1)
    {
        pbill->ChargeStartKwh = (stChTcb.stRn8209.uiE - stChTcb.stCh.uiChargeEnergy) * 10; //充电前电表读数
        pbill->ChargeStopKwh =stChTcb.stRn8209.uiE * 10;
    }
    else
    {
...
uint8_t YKC_SendBill(_GUN_NUM gun)
{
    ...
    if(DisSysConfigInfo.energymeter == 1)
    {
        YKCSendCmd3B[gun].ChargeStartKwh = (stChTcb.stRn8209.uiE - stChTcb.stCh.uiChargeEnergy) * 10; //充电前电表读数
        YKCSendCmd3B[gun].ChargeStopKwh =stChTcb.stRn8209.uiE * 10;
    }
    else
    {
...
static uint8_t   YKC_RecvPricAck(uint8_t *pdata,uint16_t len)
{
    ...
    checkYKConline = 3;    //连续空闲1个小时请求一次费率，主要是小鹤的YKC移除通道
...
static uint8_t   YKC_RecvTime(uint8_t *pdata,uint16_t len)
{
    ...
    set_date(YKCRecvCmd56.CurTime.Year + 2000,YKCRecvCmd56.CurTime.Month,YKCRecvCmd56.CurTime.Day&0x1f);
...
uint8_t  APP_YKCSENDFEILV(uint32_t timeout)
{
    ...
    static uint32_t  current_time = 0,last_time = 0;    //连续空闲3个小时或者5个小时

    if(checkYKConline == 0) //第一次进入的状态
    {
        checkYKConline = 1;
        current_time = rt_tick_get();   //清空时间
        last_time = rt_tick_get();
    }

    if(stChTcb.ucState == STANDBY)
    {
        current_time = rt_tick_get();  	//判断是否是连续空闲
    }
    else
    {
        current_time = rt_tick_get();   //不连续空闲就清空时间
        last_time = rt_tick_get();
        checkYKConline =1;
    }
    ...
    {
        if((((current_time >= last_time) ? (current_time - last_time):((0xFFFFFFFF - last_time) + current_time)) > timeout) &&(checkYKConline == 1))
        {
            last_time = current_time;
            YKC_SendPriReq();    //请求一下费率，有可能不回复,  不回复就是不断的发送
            checkYKConline = 2;
            QUENUM = 0;
        }
        else if((((current_time >= last_time) ? (current_time - last_time):((0xFFFFFFFF - last_time) + current_time)) > CH_TIME_15_SEC)&&(checkYKConline ==2))
        {
            last_time = current_time;
            YKC_SendPriReq();  //请求一下费率
            QUENUM++;
            if(QUENUM > 6)  //间隔15s，6次，90s内收到不到就会关闭网络
            {
                APP_SetNetNotConect(0);		//关闭网络操作
                checkYKConline = 1;
            }
        }
        else if(checkYKConline ==3)   //正常返回(在收到0x0A 函数里面有的)
        {
            current_time = rt_tick_get();   //正常返回标志位和时间清空
            last_time = rt_tick_get();
            checkYKConline = 1;
        }
    }
