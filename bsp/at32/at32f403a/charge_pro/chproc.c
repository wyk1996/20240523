...
#define CH_ENABLE_PIN1              40                  // GET_PIN(C,8) //98//PC8 
#define CH_ENABLE_PIN2              41                  // GET_PIN(C,9) //99//PC9 
...
uint8_t ch_is_over_current(CH_TASK_T *pstChTcb)
{
    ...
    if(rt_tick_get() - pstChTcb->stOverCurr.uiTick < CH_TIME_10_SEC)
    {
        return 0;
    }
    return 1;
...
uint8_t ch_is_over_volt(CH_TASK_T *pstChTcb)
{
    ...
    if(rt_tick_get() - pstChTcb->stOverVolt.uiTick < CH_TIME_10_SEC)
    {
        return 0;
    }
...
uint8_t ch_is_under_volt(CH_TASK_T *pstChTcb)
{
    ...
    if(rt_tick_get() - pstChTcb->stUnderVolt.uiTick < CH_TIME_10_SEC)
    {
        return 0;
    }
...
uint8_t ch_is_low_current(CH_TASK_T *pstChTcb)
{
    ...
    if(rt_tick_get() - pstChTcb->stLowCurr.uiTick <= (CH_TIME_60_SEC * 5))  //无电流时暂时修改成5分钟
    {
        return 0;
    }
...
//            && ch_is_low_current(pstChTcb))                                                   /* 持续无电流关机 amos */
    ...
    if(ch_is_low_current(pstChTcb))  //持续60s*5无电流，自动停止充电
    {
        return NO_CURRENT;
    }
