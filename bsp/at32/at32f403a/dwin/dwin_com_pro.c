...
rt_mutex_t rflash_mutex = RT_NULL;
rt_mutex_t wflash_mutex = RT_NULL;
/* 串口接收数据回调函数 */
...
_CHARGE_INFO38 ChargeInfo38;		//38页面充电信息
...
const struct fal_partition *app_code = NULL;
...
* Description  :存储当前的条数_单独一个区域存储(先擦除、后写入)
* Input        :
* Output       :None
* Return       :static
* Note(s)      :
...
* Description  :清除交易记录(先擦除、后写入) 直接写0
* Input        :
* Output       :None
* Return       :static
* Note(s)      :
...
uint8_t  APPTransactionrecord(STOP_REASON Stop_reason,_SHOW_NUM Charg_mode,_SHOW_NUM GUNnum,uint32_t CurNum)
{
    ...
    if(display3or2 == 3)
    {
        SaveRecordinfo.chargepwer = stChTcb.stCh.uiChargeEnergy;     //电量是3位小数点
    }
    else
    {
        SaveRecordinfo.chargepwer = stChTcb.stCh.uiChargeEnergy /10;   //除以10是2位小数点
    }
	
    //SaveRecordinfo.chargepwer = stChTcb.stCh.uiChargeEnergy;    //电量是3位小数点
    //SaveRecordinfo.chargepwer = stChTcb.stCh.uiChargeEnergy /10;   //除以10是2位小数点

    //SaveRecordinfo.chargepwer=((stChTcb.stCh.uiChargeEnergy /10)&0x00ff)<<8|((stChTcb.stCh.uiChargeEnergy /10) & 0xff00)>>8; //【4】充电电量0.01（4字节） //除以10是2位小数点
...
uint8_t Munu13_ShowSysInfo(void)
{
    ...
    {
        if(HY_Cover_XY == show_HY)
        {
            Dis_ShowStatus(DIS_ADD(HYMenu13.FrameID,18),SHOW_XY_HY);  //显示汇誉协议
        }
        else if(HY_Cover_XY == show_ANC)
        {
            Dis_ShowStatus(DIS_ADD(HYMenu13.FrameID,18),SHOW_HY_SHOW_AC);  //显示安充
        }
    }
    else if(NET_YX_SELCT==1)
    {
        if(YKC_Cover_XY == show_SY)
        {
            Dis_ShowStatus(DIS_ADD(HYMenu13.FrameID,18),SHOW_XY_SY); //塑云协议
        }
        else if(YKC_Cover_XY == show_YKC)
        {
            Dis_ShowStatus(DIS_ADD(HYMenu13.FrameID,18),SHOW_XY_YKC); //YKC协议
        }
        else if(YKC_Cover_XY == show_DKY)
        {
            Dis_ShowStatus(DIS_ADD(HYMenu13.FrameID,18),SHOW_XY_DKY); //DKY协议
        }
    ...
    {
        Dis_ShowStatus(DIS_ADD(HYMenu13.FrameID,18),SHOW_XY_LL); //预留1
    }
...
static uint8_t DispShow_NetState(void)
{
    ...
    {
        PrintIcon(0x2060,10);  //网络切换成单机时，图标不显示
        PrintIcon(0x2050,30);
    }
    return 1;
}
...
static void dwin_show_state(void)
{
    ...
    char * net = "https://api.huichongchongdian.com/cpile/coffee/";       /**HY二微码前缀修改安充**/
    char * ACnet = "https://www.chuangyuechongdian.com/cpile/";       /**安充二微码前缀修改**/
    char * apnet = "https://zjec.evshine.cn/scan/scan/scanTransfer?gunNo=";    /**AP  二微码前缀**/
    ...
    char * DKYnet = "https://qrcode.dazzlesky.com/scancode/connectorid/";  /**DKY二微码前缀**/
...
//				PrintStr(0x1050,DisSysConfigInfo.DevNum,16);
    ...
    {
        if(YKC_Cover_XY == show_YKC)  //显示YKC二维码
        {
            len = strlen(ykcnet);
            if(len > 80)
            {
        ...
        else if(YKC_Cover_XY == show_SY)    //显示塑云二维码
        {
            len = strlen(SYnet);
            if(len > 80)
            {
        ...
        else if(YKC_Cover_XY == show_DKY)    //显示达克云二维码
        {
            len = strlen(DKYnet);
            if(len > 80)
            {
...
//#warning hycsh "模拟金额和电量end"
        ...
        ChargeInfo.chargepwer = stChTcb.stCh.uiChargeEnergy /10; //充电电量  (原来的)==显示2位小数点
    ...
    if(CurMenu == &HYMenu38)
    {
        if(DisSysConfigInfo.energymeter == 1)
        {
            ChargeInfo38.chargecur = stChTcb.stRn8209.usCurrent;
            ChargeInfo38.chargevol = stChTcb.stRn8209.usVolt;
        }
        else
        {
        ...
        //ChargeInfo.chargepwer = stChTcb.stCh.uiChargeEnergy /10; //充电电量  (原来的)
        ChargeInfo38.chargepwer = stChTcb.stCh.uiChargeEnergy; //充电电量  (当前的)
...
void rt_dwin_state_dispose(uint8_t state)
{
    ...
    case CHARGING:	//充电中
        ...
        if((pcur != &HYMenu3) && (pcur != &HYMenu38))			//不在停止刷卡界面
        {
...
void rt_dwin_thread_entry(void *pdata)
{
    ...
    if ((app_code = fal_partition_find("app_code")) == RT_NULL)
    {
        rt_kprintf("Firmware download failed! Partition (%s) find error!", "app_code");
    }
...
//                           10);
        ...
        if(mq_service_recv_msg(CM_DISP_MODULE_ID,&stMsg,uartbuf,sizeof(uartbuf),CM_TIME_20_MSEC) == 0 )
        {
            ...
            case CM_CARD_MODULE_ID:   //卡接收  异常停止时才会进来
                if(CurMenu == NULL)
                {
                    break;
                }
            ...
            case CM_CHTASK_MODULE_ID:   /* 充电任务模块ID */
                ...
                case CH_TO_DIP_STARTFAIL:    	//启动失败
                    ...
                    if(DisSysConfigInfo.standaloneornet == DISP_NET)
                    {
                        ...
                        StoreRecodeCurNum();//启动后，记录总条数+1  暂时写到启动时
